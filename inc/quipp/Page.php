...
class Page extends Quipp {
	...
	function __construct($name) {
        ...
        // Get static page if exists
        try {
            $page = Quipp()->getPage($name);
            $this->template = $page->getTemplateFile();
            $this->info     = $page->getMetaData();
		...
		if (!$this->can_modify && isset($_GET['draft']) && $_GET['draft'] == "preview") {
			...
			if (preg_match("#[[:alnum:]]#", $name)) {
                ...
                throw new \Quipp\HTTP\Exception('Page Not Found', 404);
			}
		}
		...
		if (isset($qry)) {
			...
			if ($db->valid($res)) {	
                ...
                $check = "{$_SERVER['DOCUMENT_ROOT']}/concrete/{$name}";
                if (is_file($check)) {
                    $this->template = $check;
                    return;
                }
	...
	function print_box($class, $header, $content, $linkURL = '', $linkDisplay = '', $width = '100%', $button = false, $expandable = false)
	{
		...
		if ($expandable != false) {
			...
			$forReturn .= "<a id=\"arrow" . $myRandID . "\" style=\"cursor:pointer\"><img id=\"myTogImg".$myRandID."\" src=\"/img/arrow". $expandableArrow .".gif\" alt=\"expandContent\" /></a>";
