...
class Auth {
     ...
     * @todo Separate Authentication from Login
     * @param string
     * @param string
     */
	public function login($username, $password) {
		...
		$lQry = sprintf("SELECT itemID, sysStatus, userIDPassword
			FROM sysUsers
			WHERE userIDField  = '%s'
			AND sysOpen        = '1';",
				$this->db->escape($username)
        );
        $phRes = $this->db->query($lQry);
        if ($this->db->valid($phRes)) {
            $RS = $this->db->fetch_assoc($phRes);
            if (false == Quipp()->secure()->checkPassword($password, $RS['userIDPassword'])) {
                unset($RS);
            }
        }
			...
			if ($user->ad->authenticate(strtoupper($username), $password)) { 
        		...
        		if ($this->db->valid($lRes) || $userIsInDirectory) {
        			if (!$this->db->valid($lRes)) { //this site uses AD if this is false, this user was found in the directory, but is not in the local draggin db, so add them with no grouping
        				$qry = sprintf("INSERT INTO sysUsers (userIDField, userIDPassword, lastLoginDate, regDate, regHash, ecHash, fpHash, sysUser, sysIsADUser, sysStatus, sysOpen) VALUES ('%s', 'ACTIVE DIRECTORY MANAGED', '', %s, %s, '', '', '', '0', '1', 'active', '1');%s",
        ...
        if (isset($RS)) {
			if ($RS['sysStatus'] == 'active') {
				...
				$qry = sprintf("UPDATE sysUsers SET lastLoginDate = %s WHERE itemID = '%d';",
					$this->db->now,
					(int) $RS['itemID']);					
				$this->db->query($qry);
	...
	function check_auth() {
		...
		if (!strpos($_SERVER['PHP_SELF'], "auth.php")) {
			if (isset($_SESSION['userID']) && $_SESSION['userID']) {
				...
				$qry = sprintf("SELECT itemID, sysStatus, userIDField FROM sysUsers WHERE  itemID = '%d';",
					(int) $_SESSION['userID']);
				$res = $this->db->query($qry);
     ...
     * @param int See constants for available types
