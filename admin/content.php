...
if (isset($navRS) && isset($_GET['new']) && $_GET['new'] == 'link') {
		...
		<table class="clearTable">
			<tbody>
				...
				<tr>
					...
					print get_list("linkBehaviour", $targetOptions, "", "", $behaviour, ' class="uniform"'); ?></td>
				</tr>
				<tr>
					<td colspan="2"> 
...
<div class="boxStyle editHeader">
	<form>
		...
		<input id="pagePropertyLabel" name="pagePropertyLabel" type="text" class="formStyle pageTitle cInput" value="<?php print $pageRS['label']; ?>" />
	

	<div class="systemName">
		<label>Address (URL):</label> <?php print $_SERVER['SERVER_NAME']; ?>/
		<input id="pageSystemName" name="pageSystemName" type="text" class="formStyle systemName cInput" value="<?php print $pageRS['systemName']; ?>" />
	</div>
	
	
	</form>
	<div class="actionBtns">
		<a id="previewButton" href="/?p=<?php echo $navRS['pageSystemName']; ?>&draft=preview" target="_blank" class="btnStyle">Preview Page</a>
		...
		<table>
			<tr>
				...
				Make This Page The Home Page <br />
					<input id="makeHomepageCheck" type="checkbox" class="uniform" <?php if($pageRS['isHomepage'] == 1) { print "checked=\"checked\""; } elseif($_REQUEST['OPSet_Homepage']) { print "checked=\"checked\""; } ?>/>
				...
				<td>
						...
						if ($db->valid($res)) {
							while($grpRS = $db->fetch_assoc($res)) {
								...
								print "<label><input type=\"checkbox\" class=\"uniform\" name=\"edit_group_list[" . $grpRS['itemID'] . "]\" id=\"edit_group_list[" . $grpRS['itemID'] . "]\" value=\"" . $grpRS['itemID'] . "\"" . $checkMe . " /> " . $grpRS['nameFull'] . "</label></dd>";
		...
		<?php
			...
			if ($db->valid($tRes)) {  //grab the template data
				...
				while($tRS = $db->fetch_assoc($tRes)) {
					...
					} elseif($i > 4 || $lastGrouping != $tRS['grouping']) {
						echo "</tr><tr>";
						$i=1;
					}
...
<script>
function reloadTemplate(nonce) {
    nonce = nonce || '';
    $('#template').html('<img src="/admin/img/admin/ajax-loader.gif" />');
    var config = {
...
function reload(contentID, boxTitle, nonce)
{
    nonce = nonce || '';
	$.fancybox.close();
	$('#content_'+contentID+' a').text(boxTitle);
	reloadTemplate(nonce);
