...
if ($hasPermission && isset($_GET['sid']) && is_numeric($_GET['sid'])) {
    ...
    $fields[] = array(
        ...
        'tooltip' => "If this is not set, the class fee will be applied",
        'writeOnce' => false,
        'widgetHTML' => "<input style=\"width:300px;\" type=\"text\" class=\"uniform datepicker\" id=\"FIELD_ID\" name=\"FIELD_ID\" value=\"FIELD_VALUE\" />",
        'valCode' => "OPvalMONE",
        'dbValue' => $fee,
    ...
    switch ($_GET['view']) {
        case "edit": //show an editor for a row (existing or new)
            ...
            if (is_numeric($_GET['id'])) { //if an ID is provided, we assume this is an edit and try to fetch that row from the single table
                $qry = sprintf("SELECT tp.* FROM $primaryTableName AS tp WHERE tp.`itemID` = '%d' AND tp.`sessionID` = '%d' AND tp.`sysOpen` = '1'", (int) $_GET['id'], $sessionID
                );
                $res = $db->query($qry);
        ...
        default: //(list)
            ...
            $listqry = sprintf("SELECT cr.`itemID`, concat(cr.`lastName`, ', ' ,cr.`firstName`) AS name, cr.`email`, cr.`formDate`, 
                ...
                ORDER BY cr.`itemID` ASC, cr.`isRegistered` DESC, cr.`waitlist` ASC", (int) $db->escape($_GET['sid'], true)
            );
            ...
            if (is_object($resqry) && $resqry->num_rows > 0) {
                ...
                foreach ($registered as $dt) {
                    ...
                    echo '<td style="width:40px;"><input class="btnStyle red noPad" id="btnDelete_' . $dt['itemID'] . '" type="button" onclick="javascript:confirmDelete(\'?app='.$_GET["app"].'&sid=' . $_GET['sid'] . '&action=delete&amp;id=' . $dt['itemID'] . '\');" value="Delete"></td>';
                    echo '<td style="width:40px;"><input class="btnStyle blue noPad" id="btnEdit_' . $dt['itemID'] . '" type="button" onclick="javascript:window.location=\'?app='.$_GET["app"].'&sid=' . $_GET['sid'] . '&view=edit&amp;id=' . $dt['itemID'] . '\';" value="Edit"></td></tr>';
                }
                ...
                if (isset($waitlist)) {
                    ...
                    foreach ($waitlist as $dt) {
                        ...
                        echo '<td style="width:50px;"><input class="btnStyle red noPad" id="btnDelete_' . $dt['itemID'] . '" type="button" onclick="javascript:confirmDelete(\'?app='.$_GET["app"].'&sid=' . $_GET['sid'] . '&action=delete&amp;id=' . $dt['itemID'] . '\');" value="Delete"></td>';
