...
var update_add_edit_select_cal = function(){
	if ($('#leftColAdmin ul').length == 1){
		...
		$('#calsList span').each(function(index){
			$(':input[name=calendarID]','#frmEditEvents').append('<option value="'+$(this).attr('id').replace("cal","")+'">'+$(this).html()+'</option>');
			$(':input[name=calendarID] option:eq('+(1+index)+')','#frmEditEvents').css('color',$(this).parent().parent().css('color'));
		});
	}
}
/**
...
var update_calendar = function(form,action){
	...
	if (action == 'u'){
		...
		var params = {'itemID':$(':input[name=calendarID]',form).val(),'eventBackgroundColor':$(':input[name=backgroundColorEdit]',form).val(),'calendarName':calendarName};
	}
	else{
...
var save_add_edit_events = function(form,dialog){
	...
	$.get(ajaxRoot+'/ajax/apps/calendar/adminCalendar.php?isAjax=y&request=event&action='+$(':input[name=eventSaveType]',form).val(),(params),function(data){
		...
		if (data.match(/true/)){$('#calendar').fullCalendar('refetchEvents');}
	});
}
$(document).ready(function(){
	...
	if ($('#calendar').length == 1){
		...
		$('#dlgAddEditEvent').dialog("option","width",450);
