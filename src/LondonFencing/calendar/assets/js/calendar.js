...
var add_remove_event_sources = function(action,calendarID){
	$('#calendar').fullCalendar(action,ajaxRoot+'/calendar.php?isAjax=y&calendar='+calendarID);
}
/**
...
var page_fullCal_events_widget = function(action){
    ...
    $.get(ajaxRoot+'/calendar.php?isAjax=y&qview=list',{'view[]':calendars,'cmmd':action,'timestamp':$('#plistTimeStamp').html()},function(data){
        $('#eventsWidget').html(data);
...
*/
var show_quick_widget_events = function(timestamp){
	$.get(ajaxRoot+'/calendar.php',{qview:timestamp},function(data){
		$('#dlgQuickView').empty();
		if (data != 'false'){
	...
	if ($("#calendar").length == 1){
		...
		var defaults = {
			...
			viewDisplay: function(view){
				...
				var buttonRev = ($('.fc-header-center table tr td').length == 11)?$('.fc-header-center td:eq(3) span'):$('.fc-header-center td:eq(2) span');
				var buttonFwd = ($('.fc-header-center table tr td').length == 11)?$('.fc-header-center td:eq(7) span'):$('.fc-header-center td:eq(6) span');
				buttonRev.html('&#9668;&nbsp;'+viewName+'&nbsp;');
				buttonFwd.html('&#9658;&nbsp;'+viewName+'&nbsp;');
				
				if ($('.fc-header-center table tr td').length == 11){
					$('.fc-header-center td:eq(0) div').hide();
					$('.fc-header-center td:gt(0) div').show();
					$('.fc-header-center td:eq(10) div').hide();
				}
				//hide year buttons on day and week view
				if (view.name !== "month"){
					$('.fc-header-center td:eq(1) div').hide();
					$('.fc-header-center td:eq(9) div').hide();
				}
				toggleCalendarList(view.name);
			},
		...
		var customButtons = function(){
			...
			$('#customMonthYear').click(function(){
				...
				if ($('#dvListView').attr("class").match(/active/)){
			...
			//add two new buttons for scrolling through upcoming lists
