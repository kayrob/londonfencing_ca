...
if (!class_exists('ApprovalUtility')){
    require_once $root .'/admin/classes/ApprovalUtility.php';
}
if (!isset($approvalUtility)){
    $approvalUtility = new ApprovalUtility();
}

$permissionType = (isset($permissionType) && preg_match("%^canEdit(blog|News)$%",$permissionType,$mp)) ?$permissionType : "canEditPosts";
$newsType = (isset($newsType) && preg_match("%^(blog|news)$%",$newsType,$nt)) ? $newsType : "post";

$meta['title'] = ucwords($newsType).' Manager';
...
if ($auth->has_permission($permissionType)){
    $hasPermission = true;
}
...
if ($hasPermission) {
    ...
    $blog = new Blog\BlogAdmin($db);

    $domains = $blog->getSites($user->id);
 ...
 /*   $fields[] = array(
    ...
    $sitesWidget .= "</select>";
    $siteLabel = "Site";
    $siteTip = "The domain this article should be posted to";
...
/*  $twitterWidget = "";
    if (isset($sites)){
        $twQry = sprintf("SELECT `handle` FROM `tblTwitter` WHERE `siteID` IN (%s) AND `sysStatus` = 'active' AND `sysOpen` = '1'",
            implode(",",$sites)
        );
    ...
    if (!empty($_POST) && validate_form($_POST)) {
        ...
        case "insert":
            ...
            $qry = sprintf("INSERT INTO %s (%s, sysUserLastMod, sysDateLastMod, sysDateCreated, sysOpen, type, approvalStatus, sysUserApproved, isPublic) VALUES (%s, '%d', %s, %s, '1', '%s', '%d', %s, 1)",
    ...
    if (isset($_GET['view']) && $_GET['view'] == 'edit') {
        array_push($quipp->js['footer'], '/js/tiny_mce/jquery.tinymce.js', '/js/tiny_mce/plugins/tinybrowser/tb_tinymce.js.php');

    }
include $root. "/admin/templates/header.php";

?>
<h1><?php echo ucwords($newsType);?> Manager</h1>
<p>This allows the ability to publish articles that are viewable on the <?php echo strtolower($newsType);?> page and <?php echo strtolower($newsType);?> widget on the public front end.</p>
...
<div class="boxStyle">
    ...
    default: //(list)
        ...
        $listqry = sprintf("SELECT `itemID`, `title`, `sysStatus`, (SELECT `domain` FROM `sysSitesDomains` AS sd WHERE sd.`siteID` = $primaryTableName.`siteID` ORDER BY myOrder LIMIT 1) AS domain FROM $primaryTableName WHERE cast(sysOpen as UNSIGNED) > 0 AND `siteID` IN (%s) AND `approvalStatus` < 2 AND `isPublic` = '1' AND `type` = '%s'",
    	   implode(",",$sites),
    	   $newsType
    	);
