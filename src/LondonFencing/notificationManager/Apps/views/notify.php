...
<div class="boxStyle">
    ...
    while ($row = $db->fetch_assoc($resQry)){
        ...
        <td><input type="text" name="RQvalMAILnotify['.trim($row["itemID"]).']" value="'.trim($row["value"]).'" id="RQvalMAILnotify_'.trim($row["itemID"]).'" class="uniform"/></td>
    ...
    <input type="hidden" name="nonce" value="'.Quipp()->config('security.nonce').'" /><input type="hidden" name="dbaction" value="update" /></p></form>';
    }
    else{
